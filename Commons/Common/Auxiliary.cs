    ...
    public static class Auxiliary
    {
        ...
        public static string OrderIdString()
        {
            lock (Locker)  //lock 关键字可确保当一个线程位于代码的临界区时，另一个线程不会进入该临界区。
            {
                ...
                Thread.Sleep(100);
                return "Wine" + DateTime.Now.ToString("yyyyMMddHHmmss") + _sn.ToString().PadLeft(10, '0');
            }

        }
        #endregion

        #region 生成session的Key  字符串根
        private static readonly object PayLocker = new object();
        private static int pay_sn = 0;
        /// <summary>
        /// 生成session的Key
        /// </summary>
        /// <returns></returns>
        ...
        {
            lock (PayLocker)  //lock 关键字可确保当一个线程位于代码的临界区时，另一个线程不会进入该临界区。
            {
                if (pay_sn == int.MaxValue)
                {
                    pay_sn = 0;
                }
                else
                ...
                Thread.Sleep(100);
                return "Wine" + DateTime.Now.ToString("yyyyMMddHHmmss") + pay_sn.ToString().PadLeft(10, '0');
            }
